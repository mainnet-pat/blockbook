...
const (
	SigHashUtxos  txscript.SigHashType = 0x20
...
func RawTxInSignature(tx *wire.MsgTx, idx int, subScript []byte,
	hashType txscript.SigHashType, key *btcec.PrivateKey, amt int64, sourceOutputs *[]*wire.TxOut) ([]byte, error) {

	hash := calcBip143SignatureHash(subScript, txscript.NewTxSigHashes(tx), hashType, tx, idx, amt, sourceOutputs)
...
func SignTxOutput(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	previousScript []byte, amt int64, sourceOutputs *[]*wire.TxOut) ([]byte, error) {

	sigScript, class, addresses, nrequired, err := sign(chainParams, tx,
		idx, pkScript, hashType, kdb, sdb, amt, sourceOutputs)
	if err != nil {
		return nil, err
	}
	...
	if class == txscript.ScriptHashTy {
		...
		realSigScript, _, _, _, err := sign(chainParams, tx, idx,
			sigScript, hashType, kdb, sdb, amt, sourceOutputs)
		if err != nil {
			return nil, err
		}
...
// calcHashUtxos computes a hash digest of all UTXOS spent by the
// transaction encoded using the wire format.
func calcHashUtxos(sourceOutputs []*wire.TxOut) chainhash.Hash {
	var b bytes.Buffer
	for _, utxo := range sourceOutputs {
...
func calcBip143SignatureHash(subScript []byte, sigHashes *txscript.TxSigHashes,
	hashType txscript.SigHashType, tx *wire.MsgTx, idx int, amt int64, sourceOutputs *[]*wire.TxOut) []byte {

	if hashType&SigHashUtxos != 0 && sourceOutputs == nil {
		fmt.Printf("calcBip143SignatureHash error: SigHashUtxos set but sourceOutputs is nil")
		return nil
	}
	...
	if hashType&SigHashUtxos != 0 {
		// Write the hash of all UTXOS being spent by this transaction.
		hashUtxos := calcHashUtxos(*sourceOutputs)
		sigHash.Write(hashUtxos[:])
	}
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	subScript []byte, hashType txscript.SigHashType, kdb txscript.KeyDB, sdb txscript.ScriptDB, amt int64, sourceOutputs *[]*wire.TxOut) ([]byte,
	...
	case txscript.PubKeyHashTy:
		...
		script, err := SignatureScript(tx, idx, subScript, hashType,
			key, compressed, amt, sourceOutputs)
		if err != nil {
			return nil, class, nil, 0, err
		}
	...
	case txscript.MultiSigTy:
		script, _ := signMultiSig(tx, idx, subScript, hashType,
			addresses, nrequired, kdb, amt, sourceOutputs)
		return script, class, addresses, nrequired, nil
	default:
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType txscript.SigHashType,
	addresses []btcutil.Address, nRequired int, kdb txscript.KeyDB, amt int64, sourceOutputs *[]*wire.TxOut) ([]byte, bool) {
	...
	for _, addr := range addresses {
		...
		sig, err := RawTxInSignature(tx, idx, subScript, hashType, key, amt, sourceOutputs)
		if err != nil {
			continue
		}
...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte, hashType txscript.SigHashType, privKey *btcec.PrivateKey, compress bool, amt int64, sourceOutputs *[]*wire.TxOut) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subscript, hashType, privKey, amt, sourceOutputs)
	if err != nil {
		return nil, err
	}
