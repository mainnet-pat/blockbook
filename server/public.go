...
func (s *PublicServer) parseTemplates() []*template.Template {
	...
	if s.chainParser.GetChainType() == bchain.ChainEthereumType {
		...
		t[blockTpl] = createTemplate("./static/templates/block.html", txDetailTemplate, "./static/templates/paging.html", "./static/templates/base.html")
		if s.is.IsBCH() {
			t[bcashTokenTpl] = createTemplate("./static/templates/bcashToken.html", txDetailTemplate, "./static/templates/paging.html", "./static/templates/base.html")
	...
	if token != nil {
		...
		if token.Nft != nil {
			...
			if token.Nft.Capability != "none" {
				rv.WriteString(html.EscapeString(string(token.Nft.Capability)))
			}
			rv.WriteString(" nft")
			rv.WriteString(`</div>`)
			...
			tokenIconImg := ""
			if token.Nft.Icon != "" {
				tokenIconImg = fmt.Sprintf(`<img onerror="this.style.display='none'" src="%s" alt="Token Icon" width="32px" height="32px"> `, html.EscapeString(token.Nft.Icon))
			}
